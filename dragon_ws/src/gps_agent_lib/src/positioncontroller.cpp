...
void PositionController::update(RobotPlugin *plugin, ros::Time current_time, boost::scoped_ptr<Sample>& sample, Eigen::VectorXd &torques)
{
    ...
    // ROS_INFO("temp_angles_::rows V.S. torques.rows()  (%d vs %d)",
    //                 temp_angles_.rows(), torques.rows());
    // ROS_INFO("temp_angles_::rows V.S. current_angles_.rows()  (%d vs %d)",
    //                 temp_angles_.rows(), current_angles_.rows());
...
void PositionController::configure_controller(OptionsMap &options)
{
    ...
    // ROS_WARN("boost::get<int> (%d)", mode_);
    if (mode_ != gps::NO_CONTROL){
        ...
        // ROS_WARN("data size(r * c): (%d * %d)", data.rows(), data.cols());
        // ROS_WARN("pd_gains size(r * c): (%d * %d)", pd_gains.rows(), pd_gains.cols());
